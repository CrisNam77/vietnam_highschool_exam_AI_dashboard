...
def build_clean_run_report(summary: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        *_raw_file_lines(raw_metadata),
        ...
        f"- `diem_anh`: dùng `ngoai_ngu` khi `ma_ngoai_ngu == N1`; với các năm thiếu mã ngoại ngữ ({english_years}), tạm coi `ngoai_ngu` là tiếng Anh.",
        ...
        "- Tạm dùng `ngoai_ngu = diem_anh` cho các năm thiếu mã ngoại ngữ.",
    ...
    lines.extend(
        [
            ...
            "- Giả định tiếng Anh cho năm thiếu mã ngoại ngữ nên được rà soát khi có dữ liệu chính thức.",
